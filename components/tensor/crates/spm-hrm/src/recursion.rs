...
pub fn forward<S: WeightStream>(
    ...
    let (z_low, z_high) = states;
    let (low, high) = modules;
    ...
    for sweep_index in 0..config.h_cycles * config.l_cycles {
        ...
        sweep(groups, &config.block, low, z_low)?;
        report.low_sweeps += 1;
        if sweep_index % config.l_cycles == config.l_cycles - 1 {
            sweep(groups, &config.block, high, z_high)?;
...
/// Runs every layer of one module over its state.
