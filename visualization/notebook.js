(function (root, factory) {
  ...
  async function loadInstallGraphic() {
    ...
    try {
      ...
      target.innerHTML = `<div class="placement-callout"><strong>Demand cache breaks even at batch 2</strong><span>${(100 * physical.mean).toFixed(1)}% ±${(100 * physical.ci95_half_width).toFixed(1)} points fewer physical reads under controlled eviction</span></div><div class="metric-sheet"><h3>128 MiB demand cache <small>same policy, two memory regimes</small></h3>${placementBar("Userspace bytes", 100 * adaptive.source_byte_reduction, 15, "%", "gpu")}${placementBar("Physical bytes", 100 * physical.mean, 15, "%", "cpu")}</div><div class="metric-sheet"><h3>Arithmetic adapter <small>real Gemma block 0</small></h3>${placementBar("Cache resident", adapter.cache_resident_bytes / 1048576, 128, "MiB", "gpu")}${placementBar("Max error / tolerance", 100 * adapter.max_error / adapter.tolerance, 100, "%", "cpu")}</div><div class="placement-detail"><span>Native hook: <b>${adapter.native_span_hook.acquires.toLocaleString()} balanced leases</b> · complete logits identical</span><span>Bounded native provider: <b>not yet</b> · pass-through validation only</span></div>`;
