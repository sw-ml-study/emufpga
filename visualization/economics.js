...
(function(root){
  function analyze(x){
    ...
    const independentUnion=x.total_experts*(1-Math.pow(1-x.active_experts/x.total_experts,x.batch));
    const union=x.union_experts === undefined ? independentUnion : x.union_experts;
    ...
    return {resident,serial,budget,union,independentUnion,blind_use:x.active_experts/x.total_experts,union_use:union/x.total_experts,compute,blind:Math.min(compute,blindBandwidth),selected:Math.min(compute,unionBandwidth),block_ns:62*1000/x.clock_mhz,verdict};
  }
