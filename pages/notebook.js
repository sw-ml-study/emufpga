(function (root, factory) {
  ...
  async function loadInstallGraphic() {
    ...
    try {
      ...
      const [measured, lifetime] = await Promise.all([fetch("gemma4-profile-cache-analysis.json").then(r => r.json()), fetch("gemma4-demand-cache-break-even.json").then(r => r.json())]);
      ...
      target.innerHTML = `<div class="placement-callout"><strong>Preload loses; demand admission breaks even</strong><span>batch ${adaptive.elapsed_break_even_batch} · ${(100 * adaptive.source_byte_reduction).toFixed(1)}% less userspace copying by batch 12</span></div><div class="metric-sheet"><h3>Short cold c8 hit rate <small>preload cost still loses</small></h3>${placementBar("Static", 100 * c8static.hit_rate_p50, 30, "%", "cpu")}${placementBar("Warm", 100 * c8warm.hit_rate_p50, 30, "%", "gpu")}</div><div class="metric-sheet"><h3>Long-lived savings <small>128 MiB demand cache</small></h3>${placementBar("Userspace bytes", 100 * adaptive.source_byte_reduction, 15, "%", "gpu")}${placementBar("Physical HDD bytes", 100 * adaptive.physical_byte_reduction, 15, "%", "cpu")}</div><div class="placement-detail"><span>512 MiB bought <b>no additional hits</b></span><span>Physical savings: <b>0%</b> · Linux page cache already served later batches</span></div>`;
