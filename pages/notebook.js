(function (root, factory) {
  ...
  async function loadInstallGraphic() {
    ...
    try {
      ...
      const [measured, lifetime, direct] = await Promise.all([fetch("gemma4-profile-cache-analysis.json").then(r => r.json()), fetch("gemma4-demand-cache-break-even.json").then(r => r.json()), fetch("gemma4-direct-cache-break-even.json").then(r => r.json())]);
      ...
      target.innerHTML = `<div class="placement-callout"><strong>Demand cache breaks even at batch 2</strong><span>${(100 * physical.mean).toFixed(1)}% ±${(100 * physical.ci95_half_width).toFixed(1)} points fewer physical reads under controlled eviction</span></div><div class="metric-sheet"><h3>Short cold c8 hit rate <small>preload cost still loses</small></h3>${placementBar("Static", 100 * c8static.hit_rate_p50, 30, "%", "cpu")}${placementBar("Warm", 100 * c8warm.hit_rate_p50, 30, "%", "gpu")}</div><div class="metric-sheet"><h3>128 MiB demand cache <small>same policy, two memory regimes</small></h3>${placementBar("Userspace bytes", 100 * adaptive.source_byte_reduction, 15, "%", "gpu")}${placementBar("Physical bytes", 100 * physical.mean, 15, "%", "cpu")}</div><div class="placement-detail"><span>Buffered physical savings: <b>0%</b> · kernel cache wins</span><span>Forced-eviction savings: <b>${(100 * physical.mean).toFixed(1)}%</b> · byte replay, not inference</span></div>`;
