(function (root, factory) {
  ...
  async function loadInstallGraphic() {
    ...
    try {
      ...
      const [measured, lifetime, direct, adapter, concurrent] = await Promise.all([fetch("gemma4-profile-cache-analysis.json").then(r => r.json()), fetch("gemma4-demand-cache-break-even.json").then(r => r.json()), fetch("gemma4-direct-cache-break-even.json").then(r => r.json()), fetch("gemma4-inference-cache-adapter.json").then(r => r.json()), fetch("gemma4-concurrent-native-cache-pilot.json").then(r => r.json())]);
