(function (root, factory) {
  ...
  async function loadInstallGraphic() {
    ...
    try {
      ...
      const [measured, lifetime, direct, adapter] = await Promise.all([fetch("gemma4-profile-cache-analysis.json").then(r => r.json()), fetch("gemma4-demand-cache-break-even.json").then(r => r.json()), fetch("gemma4-direct-cache-break-even.json").then(r => r.json()), fetch("gemma4-inference-cache-adapter.json").then(r => r.json())]);
      ...
      target.innerHTML = `<div class="placement-callout"><strong>128 MiB native LRU: correct, but no source-byte savings</strong><span>${native.loads} loads and ${native.evictions} evictions expose a token reuse distance larger than this cache</span></div><div class="metric-sheet"><h3>Controlled byte replay <small>six repeated c4 batches</small></h3>${placementBar("Physical bytes saved", 100 * physical.mean, 15, "%", "cpu")}${placementBar("Userspace bytes saved", 100 * adaptive.source_byte_reduction, 15, "%", "gpu")}</div><div class="metric-sheet"><h3>Complete native inference <small>three greedy steps, CPU</small></h3>${placementBar("Owned peak", native.peak_bytes / 1048576, 128, "MiB", "gpu")}${placementBar("Source bytes saved", 100 * native.source_byte_reduction, 15, "%", "cpu")}</div><div class="placement-detail"><span>Correctness: <b>${native.acquires.toLocaleString()} balanced leases</b> · logits bit-identical</span><span>Conclusion: <b>ordinary single-request LRU loses</b> · test concurrent layer-aware reuse next</span></div>`;
